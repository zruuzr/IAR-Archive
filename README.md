<div align="center"><img src="assets/logo.svg" alt="IAR Archive Logo" width="120" height="120" />
📚 IAR Archive
Iraqi Administrative Reference Repository
مستودع رقمي لأرشفة وتنظيم المراجع الإدارية العراقية

https://img.shields.io/badge/Hosted%20on-Firebase%20Hosting-orange?style=flat&logo=firebase
https://img.shields.io/badge/Database-Firebase-yellow?style=flat&logo=firebase
https://img.shields.io/badge/PWA-Installable-blueviolet?style=flat&logo=pwa
https://img.shields.io/badge/License-All%20Rights%20Reserved-red.svg
https://github.com/zruuzr/IAR-Archive/actions/workflows/auto_process.yml/badge.svg
...
Trigger — A push event adds a new document (PDF, DOCX, XLSX, PPTX, ODT, RTF, EPUB, CSV, or ZIP) to pdf/.

Extract — ZIP archives are automatically expanded; unsafe paths are rejected.

AnyDoc Extraction — Each document is converted to clean, structured Markdown via firecrawl-anydoc. If anydoc fails on a PDF, pypdf is used as a fallback.

Scanned PDF Detection — anydoc flags fully-scanned PDFs via NeedsOcrError; these are uploaded directly to Gemini (Groq cannot process them).

AI Analysis — Extracted text is sent to Gemini 3.6 Flash to extract metadata:
...
Fallback Strategy — If Gemini fails (429 rate limit, 503 unavailable, network error), the request is automatically retried via Groq (llama-3.3-70b-versatile). If both fail, the file is skipped and retried on the next run.

Persist — Results are appended to books.json and committed back to the repository.

Deploy — GitHub Actions triggers a Firebase Hosting deploy automatically, publishing the updated site to production.
