...
(() => {
  ...
  const state = {
    ...
    loading: true,
    error: false,
    metricsLoadedIds: new Set(),
    metricsLoadingIds: new Set(),
    metricsAllLoaded: false,
  ...
  */
  ...
  function asset(value, pdf = false) {
    ...
    try {
      ...
      /*
        Covers resolve from the current deployment;
        PDFs intentionally remain on GitHub Raw.
      */
      const base = pdf
  ...
  function normalize(raw) {
    ...
    try {
      ...
      /* malformed encoding is not fatal */
    }
  ...
  const categoryKey = b =>
    b.category || '__general__';

  const categoryLabel = b =>
    field(b, 'category') || t('عام', 'General');
  ...
  async function loadRatedStatuses(ids) {
    try {
      ...
      const values = [
        ...
        id => !state.ratedLoadedIds.has(id)
      );
      ...
      const results = await Promise.allSettled(
        values.map(async id => {
          ...
          const voteDoc =
            await timeout(voteRef.get());

          if (voteDoc.exists) {
            state.ratedIds.add(id);
          } else {
            state.ratedIds.delete(id);
          }
      ...
      console.warn(
        'Unable to load private rating status:',
        error.message
      );
  ...
  async function loadAllMetrics() {
    ...
    if (
      !db ||
      state.metricsAllLoaded ||
      state.metricsAllLoading
    ) {
      return;
    }
    ...
    state.books.forEach(book => {
      state.metricsLoadedIds.add(book.id);
    });
  ...
  function visibleMetricIds() {
    ...
    return [
      ...(spotlight
        ? [spotlight.id]
        : []),
      ...page.map(book => book.id)
    ];
  }

  async function loadVisibleMetrics(
    ids = visibleMetricIds()
  ) {
    await firebaseReady;
    ...
    const needed = [
      ...
      id =>
        !state.metricsLoadedIds.has(id) &&
        !state.metricsLoadingIds.has(id)
    );
    ...
    needed.forEach(id => {
      state.metricsLoadingIds.add(id);
    });
    ...
    try {
      ...
      needed.forEach(id => {
        state.metricsLoadingIds.delete(id);
      });
    }
  }
  ...
  function toast(message, error = false) {
    ...
    if (!host) {
      ...
      /*
        Use setAttribute instead of className so this remains safe
        even if the host is changed to an SVG-compatible element.
      */
      host.setAttribute(
        'class',
        'notification-host'
    ...
    /*
      Avoid direct className assignment.
    */
    notification.setAttribute(
      'class',
      `iar-notice ${error ? 'is-error' : ''}`
    );
  ...
  const citation = book =>
    `${field(book, 'author') || labels().unknown} ` +
    `(${book.year || t('د.ت.', 'n.d.')}). ` +
    `${field(book, 'title')}. ` +
  ...
  function modal(id, open = true) {
    ...
    if (window.bootstrap?.Modal) {
      ...
      toast(
        t(
          'تعذّر تحميل مكوّن النوافذ. أعد المحاولة بعد الاتصال بالإنترنت.',
          'Dialog component unavailable. Reconnect and retry.'
        ),
        true
      );
    }
  ...
  function setPdfReaderState(mode) {
    ...
    text(
      ...
      loading
        ...
        : t(
            'تعذر فتح القارئ الداخلي. استخدم الرابط المباشر أسفل النافذة.',
            'The embedded reader could not open. Use the direct link below.'
          )
    );

    if (frame) {
      frame.hidden = loading;
    }
  }
  ...
  function readBook(book) {
    ...
    text(
      'pdfExternalLink',
      t(
        'فتح الملف في نافذة مستقلة إذا لم يظهر القارئ',
        'Open file directly in a new tab if the reader is unavailable'
      )
    );
  ...
  $('pdfReaderModal')?.addEventListener(
    'hidden.bs.modal',
    () => {
      clearTimeout(
        pdfLoadTimer
  ...
  async function download(book) {
    if (
      !book.file_path ||
      busyDownloads.has(book.id)
    ) {
      return;
    }

    busyDownloads.add(book.id);
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          /*
            metrics failure does not affect the download
          */
        }
      }
      ...
      toast(
        t(
          'تم إرسال الملف إلى المتصفح.',
          'File sent to your browser.'
        )
      );
    } catch {
      toast(
  ...
  function setDownloadBusy(
    id,
    busy
  ) {
    document
      .querySelectorAll(
        `[data-action="download"][data-id="${id}"]`
      )
      .forEach(button => {
    ...
    try {
        ...
        await db.runTransaction(
          async transaction => {
            ...
            */
            ...
            /*
              Preserve duplicate-vote protection
              for legacy aggregate documents until
              their old voters are migrated.
            */
            if (
              legacyVoters.includes(
                uid
              )
            ) {
              throw new Error(
      ...
      toast(
        error.message ===
          'ALREADY_VOTED'
          ? t(
              'سبق أن قيّمت هذا المرجع.',
              'You already rated this reference.'
            )
          : t(
  ...
  function cover(
    ...
    const fallback = `
      <span
        class="cover-placeholder designed-cover cover-tone-${color}"
        ${image ? 'hidden' : ''}
      >
    ...
    return `
      <div class="cover-container ${
        ...
        <button
          type="button"
          class="cover-img-wrapper book-cover-button"
          data-action="${
            book.cover_image
              ? 'cover'
              : 'details'
  ...
  function action(
    ...
    const disabled =
      ['read', 'download'].includes(
        name
      ) && !book.file_path;

    const pressed =
      name === 'favorite'
        ? `aria-pressed="${state.favorites.has(book.id)}"`
        : '';
      ...
      state.bundleMode
        ? `
          <div class="bundle-mode-notice">
            ...
            <button
              type="button"
              class="btn btn-iar-action"
              data-action="clear-bundle"
            >
              ${t(
                'العودة للمكتبة',
                'Back to library'
      ...
      books.length
        ...
        : `
          <div class="empty-state">
            ...
            <p>
              ${t(
                'جرّب كلمات أخرى أو أعد ضبط خيارات العرض.',
                'Try another search or reset your filters.'
              )}
            </p>

            <button
              class="btn btn-iar-primary"
              data-action="reset"
            >
              ${t(
                'إعادة ضبط البحث',
  ...
  function renderSingle() {
    ...
    if (
      ...
      $('singleReadBtn').disabled =
        !book.file_path;

      $('singleReadBtn').onclick =
        () => readBook(book);
    }
  ...
  function language() {
    ...
    const pairs = {
      ...
      'txt-subtitle': [
        'الأرشيف الإداري العراقي',
        'Iraqi Administrative Reference'
      ],
      ...
      'txt-about-desc': [
        'مساحتك لاستكشاف علوم الإدارة. اقرأ، اكتشف الأفكار، وابنِ حزمة مراجعك القادمة — كل ذلك في مكان واحد.',
        'Your space to explore management sciences. Read, discover ideas and build your next research collection — all in one place.'
      ],

      'txt-kicker': [
        'مساحة للمعرفة الإدارية',
        'A space for management knowledge'
      ],
      ...
      summaryModalTitle: [
        'ملخص المرجع · 3 دقائق',
        'Reference summary · 3 minutes'
      ],

      'txt-modal-ideas-title': [
        'أهم الأفكار',
        'Key ideas'
      ],

      'txt-modal-audience-title': [
        'الفئة المستهدفة',
        'Target audience'
      ],
    ...
    const attributes = {
      searchInput: [
        'placeholder',
        t(
          'ابحث بعنوان، مؤلف، أو فكرة…',
          'Search a title, author or idea…'
        )
      ],
      ...
      brandLockup: [
        'aria-label',
        t(
          'IAR Archive — العودة إلى المكتبة',
          'IAR Archive — Back to library'
        )
      ],
    ...
    /*
      FIX:
      Do not assign SVG/DOM className directly.
      setAttribute works safely for HTML and SVG elements.
    */
    if (backIcon) {
  ...
  function theme() {
    ...
    /*
      FIX:
      themeIcon may be an SVGElement in the deployed HTML.
      SVGElement.className is not a writable string property.
      setAttribute('class', ...) works for both HTML and SVG.
    */
    if (icon) {
      icon.setAttribute(
        'class',
  ...
  async function fetchBooks() {
    ...
    try {
      ...
      /*
        ...
        after books.json has been loaded.
        A Firebase failure must never prevent
        the library itself from rendering.
      */
      firebaseReady =
        connectFirebase();
      ...
      if (container) {
        container.innerHTML = `
          ...
          >
            ...
            <p>
              ${t(
                'تأكد من وجود books.json بجانب index.html، ثم أعد المحاولة.',
                'Ensure books.json is available next to index.html, then retry.'
              )}
            </p>
